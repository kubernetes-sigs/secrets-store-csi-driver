...
func TestWritePayloads(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := WritePayloads(dir, tc.first, constants.NO_GID); err != nil {
			...
			if err := WritePayloads(dir, tc.second, constants.NO_GID); err != nil {
...
func TestWritePayloads_BackwardCompatible(t *testing.T) {
	...
	if err := WritePayloads(dir, payload, constants.NO_GID); err != nil {
