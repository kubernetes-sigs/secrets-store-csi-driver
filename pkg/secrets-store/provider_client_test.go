...
func TestMountContent(t *testing.T) {
	...
	for _, test := range cases {
		t.Run(test.name, func(t *testing.T) {
			...
			objectVersions, _, err := MountContent(context.TODO(), client, "{}", "{}", targetPath, test.permission, nil, constants.NO_GID)
			if err != nil {
...
func TestMountContent_TooLarge(t *testing.T) {
	...
	_, errorCode, err := MountContent(context.TODO(), client, "{}", "{}", targetPath, "777", nil, constants.NO_GID)
	if err == nil {
...
func TestMountContentError(t *testing.T) {
	...
	for _, test := range cases {
		t.Run(test.name, func(t *testing.T) {
			...
			objectVersions, errorCode, err := MountContent(context.TODO(), client, test.attributes, test.secrets, test.targetPath, test.permission, nil, constants.NO_GID)
			if err == nil {
				t.Errorf("expected err to be not nil")
			}
