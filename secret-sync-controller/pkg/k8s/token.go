...
func (c *TokenClient) SecretProviderServiceAccountTokenAttrs(namespace, serviceAccountName string, tokenRequest *tokenutil.TokenRequest) (map[string]string, error) {
	// compliant with the providers
	if tokenRequest == nil {
		return map[string]string{
			"csi.storage.k8s.io/serviceAccount.tokens": "{}",
		}, fmt.Errorf("token request is nil")
	}
	...
	if tokenRequest.ExpirationSeconds == nil || *tokenRequest.ExpirationSeconds > 600 {
		tokenRequest.ExpirationSeconds = &tokenExpirationSeconds
	}
